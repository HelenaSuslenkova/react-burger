...
export const burgerConstructorElementsReducer = (state = initialState, action: any) => {
  switch(action.type) {
    ...
    case DELETE_BURGER_ELEMENT: {
      return {
        ...state,
        elements: state.elements.filter((element: BurgerIngredientType) => (element.id !== action.payload)),
      }
    }
...
function sortElements(elements: Array<BurgerIngredientType>, firstIndex: number, secondIndex: number) {
